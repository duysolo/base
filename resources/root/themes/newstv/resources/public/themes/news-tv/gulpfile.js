...
const coreScripts = [
  ...
  path.bower + '/jquery.marquee/jquery.marquee.min.js',
  path.bower + '/slick-carousel/slick/slick.min.js',
  path.bower + '/shave/dist/shave.min.js'
];
...
gulp.task('demo:styles', () => {
    return gulp.src(path.base + '/demo/**/*.scss')
        .pipe($.plumber())
        .pipe($.sass.sync({
            outputStyle: 'expanded',
            precision: 10,
        ...
        .pipe(gulp.dest(path.base + '/demo'))
        .pipe(reload({stream: true}));
});
gulp.task('demo:scripts', () => {
    return gulp.src([path.base + '/demo' + '/**/*.js', '!' + path.base + '/**/*.min.js'])
        .pipe($.uglify())
        .pipe($.babel())
        .pipe($.rename('demo.min.js'))
        .pipe(gulp.dest(path.base + '/demo'));
});
gulp.task('demo', ['demo:styles', 'demo:scripts'], () => {
    return gulp.src(path.base + '/demo' + '/**/*').pipe($.size({title: 'dist', gzip: false}));
});
...
//*** DEMO compiler task
gulp.task('build-demo', ['demo'], () => {
  return gulp.src(path.base + '/demo' + '/**/*').pipe($.size({title: 'build', gzip: true}));
});
